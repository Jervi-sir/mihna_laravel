...
@section('content')
<div class="px-50 py-50 md:px-25 md:py-25 bg-blue shadow-1 rounded-16 mt-30" style="background: #1a064f;">
    ...
    <form class="contact-form respondForm__form row y-gap-20 pt-30" method="POST" action="{{ route('instructor.login') }}">
      ...
      <div class="col-12">
          ...
          <input type="email" placeholder="Email" name="email" :value="{{ old('email') }}" value="formateur@gmail.com" required autocomplete="username" class="text-white" >
          @include('components.errors', ['messages' => $errors->get('email')])
      </div>
      <div class="col-12">
          <label class="text-16 lh-1 fw-500 text-white mb-10 ">Password</label>
          <input type="password" placeholder="Password" name="password" value="password" required autocomplete="new-password" class="text-white">
